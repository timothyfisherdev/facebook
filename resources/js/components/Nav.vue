		...
		<div class="w-1/3">
			<div class="flex">
				...
				<div class="relative ml-2">
					...
					<input type="search" name="search" class="rounded-full pl-8 w-56 bg-gray-200 h-8 focus:outline-none focus:shadow-outline text-sm" placeholder="Search Facebook" autocomplete="off" />
				</div>
			</div>
		</div>
		<div class="w-1/3 flex justify-center items-center h-full">
			<router-link to="/" class="px-6 h-full border-b-2 border-blue-500 h-full flex items-center">
			...
			<router-link v-if="! authUserLoading" :to="{ name: 'user.profile', params: { userId: authUser.id }}" class="px-6 h-full border-white h-full flex items-center">
			...
			<router-link to="/" class="px-6 h-full border-white h-full flex items-center">
